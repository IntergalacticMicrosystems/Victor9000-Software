...
uint16_t ftx_get_bytes_per_second(ftx_state_t *state)
{
    ...
    /* Convert ticks to seconds (18.2 ticks per second) */
    elapsed_secs = (state->stats.elapsed_ticks * 10UL) / 182UL;

    if (elapsed_secs == 0) {
        return 0;
    }

    return (uint16_t)(state->stats.bytes_transferred / elapsed_secs);
}
...
{
    /* BIOS tick count at 0040:006C (18.2 ticks per second) */
    return *(uint32_t __far *)MK_FP(0x0040, 0x006C);
}
