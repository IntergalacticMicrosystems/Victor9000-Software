...
class ReadyPacket:
    ...
    def pack(self) -> bytes:
        """Pack into bytes for transmission."""
        return struct.pack('<BB', Command.READY, self.status)
...
class DirEntry:
    ...
    # The Victor C library is built -zp1 (1-byte packed), so ftx_dir_entry_t is
    # name[13] + attr(1) + date(2) + time(2) + size(4) = 22 bytes, NO padding.
    # (The earlier value 24 was a never-exercised bug: directory-listing tests
    # only ever hit empty dirs, so DirEntry parsing never ran. The Victor side
    # sends/reads 22-byte entries: 3 + count*22 must stay within 1024.)
    ENTRY_SIZE = 22  # 13 + 1 + 2 + 2 + 4

    def pack(self) -> bytes:
        """Pack into bytes (22 bytes, matching C -zp1 ftx_dir_entry_t)."""
    ...
    def unpack(cls, data: bytes) -> 'DirEntry':
        ...
        attr, date, time, size = struct.unpack('<BHHI', data[13:22])
