...
class SerialConnection(Connection):
    ...
    def _blocking_receive(self) -> bytes:
        """Event-driven read for the receive thread: block on the port for the
        first byte, then drain whatever else has landed.

        read(1) returns the instant a byte arrives (kernel-driven) and otherwise
        blocks only up to the port's own read timeout, so an idle worker still
        loops but a real byte wakes it immediately - no 10 ms poll latency. This
        is the host-side win for the stop-and-wait round trips: every ACK/credit
        is seen as soon as it lands instead of up to a poll tick later.
        """
        if not (self._serial and self._serial.is_open):
            return b''
        first = self._serial.read(1)
        if not first:
            return b''          # port timeout elapsed with no data - loop
        extra = self._serial.in_waiting
        if extra:
            first += self._serial.read(extra)
        return first
