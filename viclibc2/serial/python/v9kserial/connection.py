...
class Connection(ABC):
    ...
    def _blocking_receive(self) -> bytes:
        """Read whatever has arrived, blocking briefly for the first byte.

        The old worker polled bytes_available and slept 10 ms when idle, which
        added up to 10 ms of latency to every byte arrival - and in a stop-and-
        wait protocol that lands on the critical path of every round trip. This
        default keeps a short poll for connections without an event-driven read;
        SerialConnection overrides it to block on the port (waking the instant a
        byte arrives) so there is no poll latency.
        """
        n = self.bytes_available
        if n > 0:
            return self._raw_receive(min(256, n))
        time.sleep(0.001)
        return b''
