 ...
 * LENGTH: 16-bit payload length (little-endian, max 2000)
...
/**
 * Initialize the packet protocol layer (polled byte I/O).
 *
 * Byte I/O goes straight to the SIO via ser_poll_*; there are no
 * interrupts, ring buffers, or RTS/CTS flow control (3-wire link). A
 * tight poll loop on the 8088 sustains clean RX to 38400 (the async max).
 *
 * NOTE: with no RTS/CTS gating, the peer is not paused while the caller
 * does between-packet work (e.g. a disk write in putfile). If that work
 * outlasts the SIO's 3-byte RX FIFO at the chosen baud, bytes are lost
 * and the packet's CRC check draws a NAK/retry. The sender must be paced
 * by other means (e.g. lower baud, or host-side delays) to stay reliable.
 *
...
/**
 * Alias for pkt_init(), retained for source compatibility. The library is
 * polled-only, so this is identical to pkt_init().
 * @param state Pointer to packet state structure
 * @param port Serial port (SER_PORT_A or SER_PORT_B)
 */
...
/*
 * Hot-loop wait profile, filled by the last pkt_recv_raw call when built with
 * -DPKT_PROFILE_GUARD. pkt_guard_log[i] = guard remaining when frame byte i
 * arrived (PKT_TIMEOUT_SHORT = no wait; lower = spun PKT_TIMEOUT_SHORT-guard
 * times). pkt_guard_n = number of valid entries.
 */
extern uint16_t pkt_guard_log[PKT_MAX_PAYLOAD + 3];
extern uint16_t pkt_guard_n;
#endif
