...
typedef struct {
    ...
    uint32_t file_size;         /* Bytes in the current transfer (segment length) */
    uint32_t file_offset;       /* File position the current transfer starts at
                                   (0 = whole-file; >0 = host-segmented partial) */
    ...
    uint8_t flow_ctrl;          /* Per-chunk credit flow control is active for the
                                   current receive: set when the incoming START
                                   carried FTX_FLAG_FLOWCTRL. While on, the receiver
                                   sends a READY credit after writing each chunk so
                                   the sender holds off the next one (no FIFO
                                   overrun during a slow floppy write). 0 by default
                                   (ftx_init zeroes the struct). */
    ...
    /* Application-owned disk staging buffer (see ftx_set_io_buffer). A transfer
     * is either a send or a receive - never both at once - so this one region
     * serves the read-ahead on send and the write-batching on receive. The
     * library never allocates it; the app passes it in. */
    uint8_t *io_buf;            /* staging buffer, or NULL if none registered */
    uint16_t io_bufsize;        /* its size in bytes; 0 = none */
    uint16_t io_fill;           /* receive: bytes currently staged (0..io_bufsize) */
    uint32_t io_base;           /* send: abs file offset of io_buf[0] (0xFFFFFFFF = empty) */
    uint16_t io_len;            /* send: valid bytes currently in io_buf */
} ftx_state_t;
...
/**
 * Register an application-owned disk staging buffer. The library uses it for
 * read-ahead while sending and for write-batching while receiving (a transfer
 * is only ever one direction at a time, so a single buffer serves both). Larger
 * buffers mean fewer, bigger disk operations - a big win on slow media (floppy).
 *
 * Call once after ftx_init, before any transfer. The buffer must outlive every
 * transfer made through this state. A send/receive will fail with
 * FTX_ERR_MEMORY if no buffer has been registered.
 *
 * @param state Transfer state
 * @param buf   Caller-owned buffer (must remain valid for the life of state)
 * @param size  Buffer size in bytes; must be >= FTX_CHUNK_SIZE. A multiple of
 *              FTX_CHUNK_SIZE (1024) is ideal so disk writes stay sector-aligned.
 * @return FTX_OK on success, FTX_ERR_PROTOCOL on a bad argument
 */
