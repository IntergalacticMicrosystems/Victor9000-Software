 ...
 *
 * Table-driven CRC-32 (IEEE 802.3) implementation.
 * Uses 1KB lookup table for fast calculation on 8086.
 */
...
{
    /*
     * Hot inner loop in inline asm. The portable C line
     *     crc = crc32_table[(crc ^ *data) & 0xFF] ^ (crc >> 8);
     * costs ~600 8088 cycles/byte (~120 us/byte, measured): `crc` is a uint32_t,
     * so the 8-bit-bus 8088 emulates the >>8, the 4-byte table fetch and the XOR
     * as strings of 16-bit ops (Watcom even calls a runtime helper for the
     * 32-bit shift). Here CRC lives in DX:AX, the >>8 is four register byte-moves
     * (almost free), and the table is indexed directly - ~8x faster, which is
     * what makes the getfile CRC pass cheap.
     *
     * Processed in <=32 KB batches so the inner counter stays a 16-bit CX; every
     * real caller passes far less (<=1 chunk), so this is a single pass.
     */
    while (len != 0) {
        uint16_t lo = (uint16_t)crc;
        uint16_t hi = (uint16_t)(crc >> 16);
        ...
        crc_next:
            ...
            xor  ax, word ptr crc32_table[bx]      /* ^= table[index] (low word)  */
            xor  dx, word ptr crc32_table[bx+2]    /*               (high word)   */
            loop crc_next
            mov  lo, ax
            mov  hi, dx
            pop  si
...
uint32_t ftx_crc32(const uint8_t *data, uint32_t len)
{
    ...
    crc = ftx_crc32_update(crc, data, len);
