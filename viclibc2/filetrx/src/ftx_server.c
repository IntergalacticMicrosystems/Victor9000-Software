...
int ftx_serve_one(ftx_state_t *state, ftx_progress_fn progress)
{
    ...
    if (cmd == FTX_CMD_QUERY) {
        /* Report a file's existence + current size. The host uses this to learn
         * a file's total size (to plan getfile segments) and to find the resume
         * point of a partially-transferred file. */
        ftx_query_t *q = (ftx_query_t *)state->pkt_buf;
        ftx_have_t have;
        char name[FTX_MAX_FILENAME];
        uint8_t nl = q->name_len;
        ftx_file_t h;

        if (nl >= FTX_MAX_FILENAME) nl = FTX_MAX_FILENAME - 1;
        memcpy(name, q->filename, nl);
        name[nl] = '\0';

        have.cmd = FTX_CMD_HAVE;
        have.exists = 0;
        have.size = 0;
        h = ftx_file_open(name, FTX_OPEN_READ);
        if (h != FTX_FILE_INVALID) {
            have.exists = 1;
            have.size = ftx_file_size(h);
            ftx_file_close(h);
        }
        printf("  <- query %s -> exists=%u size=%lu\n",
               name, (unsigned)have.exists, (unsigned long)have.size);
        state->last_error = ftx_send_packet(state, (uint8_t *)&have, sizeof(have));
        return FTX_SERVE_DONE;
    }
